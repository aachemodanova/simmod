...
Исследовать задачу об обедающих мудрецах с помощью программы *CPN Tools*[@lab].

# Задание

- реализовать задачу об обедающих мудрецах в *CPN Tools*[@cpn];
...
- В графе 11 узлов и 30 дуг, соответственно 11 состояния и 30 переходов.
- Затем указаны границы значений для каждого элемента: philosopher_eats (максимум - 2, минимум - 0), philosopher_thinks (максимум - 5, минимум - 3), sticks_on_the_table (максимум - 5, минимум - 1).
- Также указаны границы мультимножеств.
- Маркировка home равная all показывает, что мы можем перейти в любое состояние из любого другого.
- Маркировка dead равная None показывает, что нет состояний, из которых невозможен переход.
- В конце указано, что put_sticks и take_sticks повторяется бесконечное количетсво раз.
