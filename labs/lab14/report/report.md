...
- `QUEUE`=`operator_q` -- имя объекта типа «очередь»;
- `MAX`=82 -- в очереди находилось не более 82 ожидающих заявок от клиента;
- `CONT`=82 -- на момент завершения моделирования в очереди было 82 клиента;
...
Имена, используемые в программе модели: `operator`, `operator_q`, `custnum`, `fin`, `waittime`. 
...
Также появилась таблица с информацией для гистограмы: частотность разделена на 15 частотных интервалов с шагом 2 и началом в 0(как мы и задавали). Наибольшее количество заявок(17) ожидало 10-12 минут, чуть меньше (14) -- 12-14 минут, ещё меньше(12) -- 8-10 минут, в остальных диапазонах 0-4 заявки. Среднее время ожидания = 10.709 минут.

В конце отчёта идёт информация о будущих событиях:

- `XN`=98 -- порядковый номер заявки от клиента, ожидающей поступления для оформления заказа у оператора;
- `PRI`=0 -- все клиенты (из заявки) равноправны;
- `BDT`=341,236 -- время назначенного события, связанного с данным транзактом;
- `ASSEM`=98 -- номер семейства транзактов;
...
Частотность разделена на 15 частотных интервалов с шагом 2 и началом в 0(как мы и задавали). Наибольшее количество заявок(17) ожидало 10-12 минут, чуть меньше (14) -- 12-14 минут, ещё меньше(12) -- 8-10 минут, в остальных диапазонах 0-4 заявки. Среднее время ожидания = 10.709 минут.
...
- количество транзактов, вошедших в блок певрого типа заказов с начала процедуры моделирования `ENTRY COUNT` = 32, а второго типа(с дополнительными услугами) `ENTRY COUNT` = 15; обарботано 12+27 = 39 заяовк;
...
Имена, используемые в программе модели: `operator`, `operator_q`, `dop`, `notdop`. 

- количество транзактов, вошедших в блок заказов с начала процедуры моделирования `ENTRY COUNT` = 33(их стало меньше, так как раньше второго типа заказов было почти 50% и они генерировались дополнительно к обычным, а теперь это они "выбираются" из обычных), при этом из них второго типа(с дополнительными услугами) `ENTRY COUNT` = 8, обарботано 32 заказа;
...
Затем идёт информация об многоканальном устройстве `STORAGE` (оператор, оформляющий заказ), откуда видим, что к операторам попало 93 заказа от клиентов (значение поля `OWNER`=93). Полезность работы операторов составила 0,482. При этом среднее время занятости оператора составило 1,926 мин. Также появились значения, характерные для `STORAGE`: вместительность 4, максимум не занято 4 оператора, минимум -- 0.
...
- количество транзактов, вошедших в блок заказов с начала процедуры моделирования `ENTRY COUNT` = 94; обработано 60 заказа;
...
- `XN`=96 -- порядковый номер заявки от клиента, ожидающей поступления для оформления заказа у оператора;
- `PRI`=0 -- все клиенты (из заявки) равноправны;
- `BDT`=480,736 -- время назначенного события, связанного с данным транзактом;
- `ASSEM`=96 -- номер семейства транзактов;
- `CURRENT`=0 -- номер блока, в котором находится транзакт;
- `NEXT`=1 -- номер блока, в который должен войти транзакт.
